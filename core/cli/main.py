...
def main():
    ...
    scrape_parser = subparsers.add_parser("scrape", help="Run interactive chat scraping & ingestion wizard")
    scrape_parser.add_argument("--source", type=str, choices=["raw", "antigravity", "android-studio", "all"], help="Direct ingestion source")
    scrape_parser.add_argument("--path", type=str, help="Custom path for raw chats or brain directory")
    
    import_parser = subparsers.add_parser("import", help="Alias for `aura scrape` (Ingests chat files)")
    import_parser.add_argument("--source", type=str, choices=["raw", "antigravity", "android-studio", "all"], help="Direct ingestion source")
    import_parser.add_argument("--path", type=str, help="Custom path for raw chats or brain directory")
    ...
    if args.command == "init":
        ...
        if args.source == "raw":
            scrape_wizard.ingest_raw_drops(custom_path=Path(args.path) if args.path else None)
