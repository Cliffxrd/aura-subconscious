...
class AuraWizard:
    ...
    def __init__(self, target_home: Path = None):
        ...
        self.communication_style = "High-bandwidth, collaborative, concise, no robotic filler"
    ...
    def run_wizard(self, interactive: bool = True):
        ...
        if interactive:
            ...
            inp_user = input(f"? Your Name / GitHub Handle (Default: Cliffxrd): ").strip()
            if inp_user:
            ...
            inp_stack = input(f"\n? Preferred Tech Stack (Default: {self.tech_stack}): ").strip()
            if inp_stack:
                self.tech_stack = inp_stack
                
            # 4. Communication Style
            inp_style = input(f"\n? Communication Style (Default: {self.communication_style}): ").strip()
            if inp_style:
